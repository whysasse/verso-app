...
- **Phase C (after launch) — polish backlog.** Highlighting (FAB-54) done 2026-09-01 — see `docs/DONE.md`. Its follow-up FAB-303 (highlighting v2) is a parent issue whose original 5 steps all shipped 2026-09-01, but it stays open — two named follow-ups remain (headings/lists/blockquotes joining selectable regions; merging with an existing highlight) — see `docs/BACKLOG.md`. RSVP reading mode (FAB-277) and VoiceOver progress announcement (FAB-278) still need a UX decision from Fabio before implementation.
