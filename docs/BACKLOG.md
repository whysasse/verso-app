...
- **Phase D — design critique remediation (new, 2026-09-01/03).** FAB-304 (backgrounding corrupts app state) turned out to have three independent causes — the first fixed and shipped (PR #360), the second and third fixed pending Fabio's device confirmation; see its BACKLOG entry for the full history. FAB-305–333 come from [DESIGN_CRITIQUE_2026-09-01.md](DESIGN_CRITIQUE_2026-09-01.md); its §10 ranks them. Screenshot coverage as of 2026-09-03: Paper, Sepia, Ink and Night all seen, onboarding seen, immersive seen, one large-text pass done. Still unseen: iPhone SE (Fabio to capture), and the immersive-mode Back-button repro (FAB-307), which no screenshot can answer.
  ...
  *Pre-submission — fix or ship before the final binary submission (FAB-150):*
  ...
  1. **FAB-304** — theme-switch blank screen (cause 1, shipped PR #360) turned out to have two further independent causes (reader content blanking after the app switcher; backgrounding corrupting Core Data's article cache) — both fixed pending device confirmation, see its BACKLOG entry.
...
- [ ] 🔴 **FAB-304** · Backgrounding the app corrupts app state: empty article list, lost reading progress, broken scroll restore  `In Progress` `Urgent`
  ...
  **Fix:** [`ArticleLibraryService.swift`](<Verso/Sources/Services/ArticleLibraryService.swift>)'s `rebuildCache` now brackets its own access (`startAccessingSecurityScopedResource()` / `stopAccessingSecurityScopedResource()` via `defer`), the same pattern every sibling call site already uses — self-sufficient instead of depending on ambient state set up elsewhere. Belt-and-suspenders: the stale-record cleanup now also refuses to run when the parsed read comes back empty while Core Data already has cached rows (`readLooksValid = !parsedArticles.isEmpty || existing.isEmpty`), so *any* other way a folder read can fail (permission revoked, iCloud volume briefly unreachable, a transient I/O error) can no longer be misread as "the user deleted every article." The one case this doesn't cover — a folder genuinely emptied by the user through Finder/Files while nothing was reading from it — is indistinguishable from a bad read by design, so the cache is left alone rather than risk a repeat of this bug; documented inline in `ArticleLibraryService.swift`.
  ...
  `xcodebuild build` (Verso scheme) succeeds; `xcodebuild test` (VersoTests, 198 tests) passes, including the new `ArticleLibraryServiceTests`. Real confirmation is Fabio's part — re-run the 2026-09-04 repro: open an article, background via the app switcher, return (list must not go empty; reopening the same article must land near the saved position, not near the bottom); then switch themes at the list and confirm a "Continue Reading" article stays there. Also re-run the original theme-switch matrix (Ink→Paper, Sepia→Night crossing light/dark; Ink→Night, Paper→Sepia as a control) to confirm cause 1 stays fixed, and cause 2's background/foreground-while-reading repro (body renders immediately, no tap/scroll needed).
