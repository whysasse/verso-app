...
- **Phase C — post-launch polish.** FAB-54 (highlighting) done 2026-09-01 (see [DONE.md](DONE.md)); its follow-up FAB-303 (highlighting v2 — cross-block selection, formatting-aware spans, headings/lists/quotes) had all 5 of its original steps shipped 2026-09-01, but stays open — two follow-ups remain, both named in its own checklist below: headings/lists/blockquotes joining paragraphs' shared selectable regions, and merging a new selection with an adjacent/overlapping existing highlight. FAB-277 (RSVP mode), FAB-278 (VoiceOver progress announcement) still need a UX decision from Fabio before implementation starts.
...
- [ ] 🟡 **FAB-303** · [Phase 3] Highlighting v2: cross-block selection, formatting-aware spans, headings/lists/quotes  `In Progress` `Medium`
  ...
  * Selection overlapping or directly adjacent to an existing highlight → strip those markers and emit **one merged pair**. **Deferred to land alongside Step 5** (see checklist above) — still declines today, unchanged from before this step.
