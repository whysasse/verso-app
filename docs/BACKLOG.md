...
- **Phase C — post-launch polish.** FAB-54 (highlighting) done 2026-09-01 (see [DONE.md](DONE.md)); its follow-up FAB-303 (highlighting v2 — cross-block selection, formatting-aware spans, headings/lists/quotes) had all 5 of its original steps shipped 2026-09-01, plus both follow-ups named after — headings/lists/blockquotes joining selectable regions, and merging a new selection with an existing highlight (same-block only) — but stays open for one last piece, named in its own checklist below: blockquote's colored accent bar. FAB-277 (RSVP mode), FAB-278 (VoiceOver progress announcement) still need a UX decision from Fabio before implementation starts.
